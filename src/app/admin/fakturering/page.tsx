...
import { getOrdersVisibleToSM, statusLabels } from '@/data/mockData'
import { 
  Download, 
  FileText,
  DollarSign,
...
export default function AdminBillingPage() {
  const orders = getOrdersVisibleToSM()
  
  // Filter orders that are completed and ready for billing
  const completedOrders = orders.filter(o => o.status === 'utfort')
  const billedOrders = orders.filter(o => o.status === 'fakturert')
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="p-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-semibold text-brand-dark">{billedOrders.length}</p>
              <p className="text-sm text-gray-500">Fakturert</p>
            </div>
          </div>
        </Card>
          ...
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="text-right px-6 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Kjøretid</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {completedOrders.map((order) => (
                <tr key={order.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className="text-sm text-gray-600">{order.partnerName}</span>
                  </td>
                  ...
                  <td className="px-6 py-4 text-right">
                    <span className="text-sm font-medium text-gray-700">{order.driveTime || 0}</span>
                  </td>
